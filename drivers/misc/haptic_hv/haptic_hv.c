			    ...
			    struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "f0_cali_data = %d\n",
...
static int haptic_get_cmdline_f0_data(void)
{
    char *ptr = NULL;
    int f0_data = 0;
    ptr = strstr(haptic_get_cmd(),"haptic_f0=");
    if(ptr == NULL)
    {
...
static int haptic_get_cmdline_cali_data(void)
{
    char *ptr = NULL;
    int aw_cali_data = 0;
    ptr = strstr(haptic_get_cmd(),"aw_cali_data=");
    if(ptr == NULL)
    {
        aw_err("%s get null haptic_get_cmd = %s\n", __func__, haptic_get_cmd());
        return -1;
...
static int aw_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if ((strlen(current_line_motor_info.chip) > 0) && (!strcmp(current_line_motor_info.chip, "RT6010")))
	{
		pr_err("%s: RT6010 exist aw i2c ignore!\n", __func__);
		return -EIO;
	}
