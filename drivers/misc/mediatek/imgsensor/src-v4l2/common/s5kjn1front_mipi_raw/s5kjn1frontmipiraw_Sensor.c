...
#define PFX "s5kjn1front_camera_sensor"
...
#include "s5kjn1frontmipiraw_Sensor.h"
#include "s5kjn1front_ana_gain_table.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1FRONT_SENSOR_ID,//0x38e1
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[9] = {
	...
	{ 8160, 6144,    0,    0, 8160, 6144, 4080, 3072,  120, 456, 3840, 2160,    0,    0, 3840, 2160}, // custom4
};
...
kal_bool s5kjn1front_long_exposure_status = 0;
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter, kal_bool gph)
{
    ...
    if (shutter >= (imgsensor_info.max_frame_length - imgsensor_info.margin))
    {
        ...
        s5kjn1front_long_exposure_status = 1;
    ...
    else if (s5kjn1front_long_exposure_status == 1)
    {
        s5kjn1front_long_exposure_status = 0;
...
static void custom4_setting(struct subdrv_ctx *ctx)
{
	...
	table_write_cmos_sensor(ctx, addr_data_pair_custom4_jn1,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(ctx, 0x0000) << 8)
				      | read_cmos_sensor_8(ctx, 0x0001)) + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("s5kjn1front i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				/* preload 4cell data */
				read_4cell_from_eeprom(ctx, NULL);
				return ERROR_NONE;
			}
			printk("s5kjn1front Read sensor id fail, id: 0x%x, sensor id: 0x%x\n",
				ctx->i2c_write_id,*sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static int open(struct subdrv_ctx *ctx)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = (
		(read_cmos_sensor_8(ctx, 0x0000) << 8) | read_cmos_sensor_8(ctx, 0x0001)) + 1;

			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_DEBUG("s5kjn1front open i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, sensor_id);
				break;
			}

			LOG_DEBUG("s5kjn1front open Read sensor id fail, id: 0x%x, sensor id: 0x%x\n",
				ctx->i2c_write_id,sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_DVDD1, 1100000, 0},
	{HW_ID_MCLK_DRIVING_CURRENT, 8, 1},
	{HW_ID_RST, 1, 5},
};

const struct subdrv_entry s5kjn1front_mipi_raw_entry = {
	.name = "s5kjn1front_mipi_raw",
	.id = S5KJN1FRONT_SENSOR_ID,
	.pw_seq = pw_seq,
