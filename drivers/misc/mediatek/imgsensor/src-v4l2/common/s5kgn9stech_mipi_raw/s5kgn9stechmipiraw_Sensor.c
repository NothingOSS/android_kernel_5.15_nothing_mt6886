...
#define PFX "s5kgn9stech_camera_sensor"
...
#include "s5kgn9stechmipiraw_Sensor.h"
#include "s5kgn9stech_ana_gain_table.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KGN9STECH_SENSOR_ID,//0x38e1
...
kal_bool stech_long_exposure_status = 0;
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter, kal_bool gph)
{
    ...
    else if (stech_long_exposure_status == 1)
    {
        stech_long_exposure_status = 0;
        write_cmos_sensor(ctx, 0x0340, ctx->frame_length & 0xFFFF);
...
#define EEPROM_STECH_MOLDULEID 0x2
...
//	kal_uint16 sp8spFlag = 0;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(ctx, 0x0000) << 8)
			...
			if ((*sensor_id == imgsensor_info.sensor_id) && (module_id == EEPROM_STECH_MOLDULEID)) {
				LOG_DEBUG("stech s5kgn9stech write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				/* preload 4cell data */
				read_4cell_from_eeprom(ctx, NULL);
				return ERROR_NONE;
			}
			LOG_DEBUG("stech s5kgn9stech sensor id fail, id: 0x%x,sensor id: 0x%x,module_id: 0x%x\n",
	...
	if ((*sensor_id != imgsensor_info.sensor_id) ||
        (module_id != EEPROM_STECH_MOLDULEID)) {
	/* if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF */
		*sensor_id = 0xFFFFFFFF;
		LOG_DEBUG("s5kgn9 stech read sensor id fail");
...
static int open(struct subdrv_ctx *ctx)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		(read_cmos_sensor_8(ctx, 0x0000) << 8) | read_cmos_sensor_8(ctx, 0x0001)) + 1;
			...
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_DEBUG("stech s5kgn9stech open i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, sensor_id);
				break;
			}

			LOG_DEBUG("stech s5kgn9stech open Read sensor id fail, id: 0x%x,sensor id: 0x%x\n",
				ctx->i2c_write_id,sensor_id);
			retry--;
		} while (retry > 0);
		i++;
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
  ...
  case SENSOR_FEATURE_GET_GAIN_RANGE_BY_SCENARIO:
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 1560000;
		break;
...
const struct subdrv_entry s5kgn9stech_mipi_raw_entry = {
	.name = "s5kgn9stech_mipi_raw",
	.id = S5KGN9STECH_SENSOR_ID,
	.pw_seq = pw_seq,
