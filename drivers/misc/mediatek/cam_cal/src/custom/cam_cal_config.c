		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if(IMX615_SENSOR_ID == pCamCalData->sensorID){
		...
		addr_3100k = 0x15;
		AWBConfig = 1;
	}else if(S5KJN1FRONT_SENSOR_ID == pCamCalData->sensorID){
		addr_5100k = 0x4D;
		addr_4000k = 0x31;
		addr_3100k = 0x15;
		AWBConfig = 1;
	...
	if (0x1 & AWBConfig) {
		...
		if (read_data_size > 0)	{
			...
			if(S5KJN1_SENSOR_ID == pCamCalData->sensorID || IMX615_SENSOR_ID == pCamCalData->sensorID || S5KJN1FRONT_SENSOR_ID == pCamCalData->sensorID){
