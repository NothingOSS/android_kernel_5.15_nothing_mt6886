...
static u32 is_blocked_cnt;
	...
	if (req_sta_mix & (0x1 << 0))
		log_size += scnprintf(log_buf + log_size,
			LOG_BUF_SIZE - log_size, "peri ");
	...
	if (req_sta_mix & (0x1 << 4))
		log_size += scnprintf(log_buf + log_size,
			LOG_BUF_SIZE - log_size, "infra ");
...
#undef LOG_BUF_SIZE
	...
	if (wakesrc->is_abort != 0) {
		...
		log_size += scnprintf(log_buf + log_size,
			  LOG_BUF_OUT_SZ - log_size,
			  "req_sta =  0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x, cg_check_sta =0x%x, isr = 0x%x, rt_req_sta0 = 0x%x rt_req_sta1 = 0x%x rt_req_sta2 = 0x%x rt_req_sta3 = 0x%x dram_sw_con_3 = 0x%x, ",
			  wakesrc->req_sta0, wakesrc->req_sta1,
			  wakesrc->req_sta2, wakesrc->req_sta3,
			  wakesrc->req_sta4, wakesrc->req_sta5, wakesrc->req_sta6,
			  wakesrc->req_sta7, wakesrc->req_sta8,
