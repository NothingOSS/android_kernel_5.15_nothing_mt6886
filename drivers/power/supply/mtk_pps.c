...
static bool pps_is_enabled(struct pps_algo_info *info)
{
	bool en = false;
	int ret;

	pps_hal_is_enabled(info->alg, to_chgidx(PPS_DVCHG_MASTER), &en);
	if (!en) {
		ret = pps_hal_enable_charging(info->alg, to_chgidx(PPS_DVCHG_MASTER), true);
		if (ret < 0) {
			PPS_ERR("en chg fail(%d)\n", ret);
		}
	}
	PPS_INFO("dvchg is en(%d)!\n", en);
	return en;
}
