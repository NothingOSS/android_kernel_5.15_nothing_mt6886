...
static inline int to_chgclass_adc(enum pps_adc_channel chan)
{
	switch (chan) {
	case PPS_ADCCHAN_VBUS:
		return ADC_CHANNEL_VBUS;
	case PPS_ADCCHAN_IBUS:
		return ADC_CHANNEL_IBUS;
	case PPS_ADCCHAN_VBAT:
		return ADC_CHANNEL_VBAT;
	case PPS_ADCCHAN_IBAT:
		return ADC_CHANNEL_IBAT;
	case PPS_ADCCHAN_TBAT:
		return ADC_CHANNEL_TBAT;
	case PPS_ADCCHAN_TCHG:
		return ADC_CHANNEL_TEMP_JC;
	case PPS_ADCCHAN_VOUT:
		return ADC_CHANNEL_VOUT;
	case PPS_ADCCHAN_VSYS:
...
int pps_hal_get_ta_output(struct chg_alg_device *alg, int *mV, int *mA)
{
	int ret;
...
int pps_hal_get_ta_status(struct chg_alg_device *alg,
...
int pps_hal_set_ta_cap(struct chg_alg_device *alg, int mV, int mA)
{
	int ret;
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_is_ta_cc(struct chg_alg_device *alg, bool *is_cc)
{
	int ret;
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_ta_wdt(struct chg_alg_device *alg, u32 ms)
{
	int ret;
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_enable_ta_wdt(struct chg_alg_device *alg, bool en)
{
	int ret;
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_enable_ta_charging(struct chg_alg_device *alg, bool en, int mV,
				int mA)
{
	int ret;
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_sync_ta_volt(struct chg_alg_device *alg, u32 mV)
{
	int ret;
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_authenticate_ta(struct chg_alg_device *alg,
			     struct pps_ta_auth_data *data)
{
	...
	for (i = 0; i < hal->support_ta_cnt; i++) {
		...
		PPS_INFO("lmt(%d,%dW),step(%d,%d),cc=%d,cap=%d,status=%d\n",
...
int pps_hal_send_ta_hardreset(struct chg_alg_device *alg)
{
	int ret;
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_init_hardware(struct chg_alg_device *alg, const char **support_ta,
			   int support_ta_cnt)
{
	...
	data->is_dvchg_exist[PPS_DVCHG_MASTER] = true;
	if (hal->chgdevs[MTK_CHGTYP_DVCHG_SLAVE])
		data->is_dvchg_exist[PPS_DVCHG_SLAVE] = true;
...
int pps_hal_enable_charging(struct chg_alg_device *alg, enum chg_idx chgidx,
			     bool en)
{
...
int pps_hal_enable_hz(struct chg_alg_device *alg, enum chg_idx chgidx, bool en)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_vbusovp(struct chg_alg_device *alg, enum chg_idx chgidx,
			 u32 mV)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_ibusocp(struct chg_alg_device *alg, enum chg_idx chgidx,
			 u32 mA)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_vbatovp(struct chg_alg_device *alg, enum chg_idx chgidx,
			 u32 mV)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_ibatocp(struct chg_alg_device *alg, enum chg_idx chgidx,
			 u32 mA)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_vbatovp_alarm(struct chg_alg_device *alg, enum chg_idx chgidx,
			       u32 mV)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_reset_vbatovp_alarm(struct chg_alg_device *alg,
				 enum chg_idx chgidx)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_vbusovp_alarm(struct chg_alg_device *alg, enum chg_idx chgidx,
			       u32 mV)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_reset_vbusovp_alarm(struct chg_alg_device *alg,
				 enum chg_idx chgidx)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_ieoc(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mA)
{
	int ret;
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
	...
	ret = charger_dev_set_eoc_current(hal->chgdevs[chgtyp], milli_to_micro_2(mA));
	PPS_DBG("%s idx:%d %d\n", __func__, chgidx, mA);
	return ret;
}

static int pps_get_tbat(struct pps_hal *hal)
{
	int ret = 0;
	int tmp_ret;
	...
	if (IS_ERR_OR_NULL(bat_psy)) {
		PPS_ERR("%s Couldn't get bat_psy\n", __func__);
		ret = 0;
	} else {
...
static int pps_get_ibat(struct pps_hal *hal)
{
	int ret = 0;
	int tmp_ret;
	union power_supply_propval prop;
	...
	if (IS_ERR_OR_NULL(bat_psy)) {
		...
		tmp_ret = power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_CURRENT_NOW,
						&prop);
		ret = micro_to_milli_2(prop.intval);
	}
...
static int pps_get_vbat(struct pps_hal *hal)
{
	int ret = 0;
	int tmp_ret;
	union power_supply_propval prop;
...
int pps_hal_get_adc(struct chg_alg_device *alg, enum chg_idx chgidx,
		     enum pps_adc_channel chan, int *val)
{
	...
	if (_chan == ADC_CHANNEL_TBAT) {
		*val = pps_get_tbat(hal);
		return 0;
	}
	if (_chan == ADC_CHANNEL_IBAT) {
		*val = pps_get_ibat(hal);
		return 0;
	}
	if (_chan == ADC_CHANNEL_VBAT) {
		*val = pps_get_vbat(hal);
		return 0;
	}
	...
	if (_chan == ADC_CHANNEL_VBUS || _chan == ADC_CHANNEL_IBUS ||
		...
		*val = micro_to_milli_2(*val);
	return 0;
}

int pps_hal_get_soc(struct chg_alg_device *alg, u32 *soc)
{
	int ret;
	int ret_tmp;
	struct power_supply *bat_psy;
...
int pps_hal_is_pd_adapter_ready(struct chg_alg_device *alg)
{
	struct pps_hal *hal;
	int type = 0;
	int i;
...
int pps_hal_set_ichg(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mA)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_set_aicr(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mA)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_get_ichg(struct chg_alg_device *alg, enum chg_idx chgidx, u32 *mA)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
	int ret, uA = 0;
	...
	*mA = micro_to_milli_2(uA);

	return ret;
}

int pps_hal_get_aicr(struct chg_alg_device *alg, enum chg_idx chgidx, u32 *mA)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
	int ret, uA = 0;
...
int pps_hal_is_vbuslowerr(struct chg_alg_device *alg, enum chg_idx chgidx,
			   bool *err)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_get_adc_accuracy(struct chg_alg_device *alg, enum chg_idx chgidx,
			      enum pps_adc_channel chan, int *val)
{
	int ret;
	int chgtyp = to_chgtyp(chgidx);
...
int pps_hal_init_chip(struct chg_alg_device *alg, enum chg_idx chgidx)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_dump_registers(struct chg_alg_device *alg, enum chg_idx chgidx)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_set_operating_mode(struct chg_alg_device *alg, enum chg_idx chgidx,
			    bool en)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pps_hal_is_enabled(struct chg_alg_device *alg, enum chg_idx chgidx,
			   bool *en)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pps_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
