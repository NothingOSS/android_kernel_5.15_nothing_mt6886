...
int send_tfa_cal_in_band(void *buf, int cmd_size)
{
	(void)buf;
	(void)cmd_size;
	pr_info("[%s]you need to define this func in other module!\n", __FUNCTION__);
	return 0;
}
	...
	char *buffer, size_t count)
{
	...
	pr_info("[0x%x] dsp_msg buffer:%x count: %d\n", i2c->addr, buffer, count);
...
static DEVICE_ATTR(version, S_IRUGO, tfa98xx_attr_version_show, NULL);
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("tfa98xx_version_show: %s\n", buf);
	return ret;
}

static DEVICE_ATTR(lib_version, S_IRUGO, tfa98xx_attr_lib_version_show, NULL);
...
//	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
	...
	if (sscanf(buf, "%d", &value) == 1) {
		if(value == 1){
				...
				pr_info("i2c addr is 0x%x,Calibration impedenc: %d\n", tfa98xx_global[i]->i2c->addr,cal_impedence[i]);
				}
				return count;
		}else {
			return -EINVAL;
	...
	struct device_attribute *attr, char *buf)
{
	...
	char memtrack[32] = {
	    ...
	    0x22, 0x00, 0x13, 0x22, 0x00, 0x14, // req f0
        0x22, 0x00, 0x04, 0x22, 0x00, 0x05  // speak template
	};
	...
	pr_info("tfa98xx_f0_show:");
	pr_info("Fres 1:%u,2:%u\n", f0_array[0], f0_array[1]);
    pr_info("tfa98xx_f0_show end\n");
	return ret;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("tfa98xx_bypass_store: %d\n", tfa98xx_bypass);
	return count;
}
                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
    ...
    mutex_lock(&tfa98xx_mutex);
    list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		pr_info("tfa98xx: set_stereo_ctl %d, 0x%x\n", selector, tfa98xx->i2c->addr);
                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
    ...
    mutex_lock(&tfa98xx_mutex);
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
			pr_info("tfa98xx: tfa98xx_get_stereo_ctl %d, 0x%x\n", tfa98xx->flags, tfa98xx->i2c->addr);
...
retry:
	...
	pr_info("  WR-RAW (len=%d) Error I2C send size mismatch %d\n", len, ret);
	error = Tfa98xx_Error_Fail;
		...
		if(tfa98xx->tfa->cnt->nliveData > 0) {
			...
			for(i=0;i<length;i++) {
				...
				if(order[i].isMemtrackItem) {
					pr_info("tfa98xx data[%d] = %d, factor = %d\n",j,data[j] , mRecord->scalingFactor[i]);
			...
			for(j=0; j<lData->length-1u; j++) {
				...
				if (liveD->addrs[0] >= '0' && liveD->addrs[0] <= '9') {
					if (compare_strings(liveD->name, "buildinmemtrack", 25)) {
						pr_info("Found keyword: %s. Skipping the SetMemtrack! \n", liveD->name);
						skip_set_msg = 1;
					} else {
	...
	for (i=0; i < devcnt; i++ ) {
		...
		if(tfa98xx_device_count == 1) {
			if (spkr_count == 1) {
				...
				pr_info("Current f0 value: %dHz, %d\n", live_data[fResNo],fResNo);
			}
			else if(tfa98xx_device_count == 2){
				gf0[0] = live_data[fResNo];
				gf0[1] = live_data[fResNo+1];
				pr_info("speaker number is two, Current 0x34 f0 value: %dHz,Current 0x35 f0 value: %dHz,Current fResNo:%d \n",gf0[0],gf0[1],fResNo);
			}
			else{
		...
		if(tfa98xx_device_count == 1) {
			if (spkr_count == 1) {
				...
				pr_info("Current temp value: %d C\n", gtemp[0]);
			}
			else if(tfa98xx_device_count == 2){
				gtemp[0] = live_data[4]*1000;
				gtemp[1] = live_data[5]*1000;
				pr_info("speaker number is two, Current 0x34 temp value: %d C,Current 0x35 temp value: %d C\n",gtemp[0],gtemp[1]);
			}
			else{
				pr_err("spk wrong number\n");
			}
...
static Tfa98xx_Error tfa98xx_rpc_send_nondsp(struct tfa98xx *tfa98xx)
{
	...
	for(i=0; i<sizeof(bytes); i++)
		pr_info("tfa98xx_rpc_send_nondsp,bytes[%d]: 0x%x\n", i, bytes[i]);
...
/* Clear MTPOTC to make sure calibration values are always read from DSP */
	...
	for (i=0; i < 50; i++ ) {
		...
		*/
		if (buffer[2] & 0x6) {
			if (buffer[2] & 0x2)
				pr_info("%s: Left SPK damaged event detected 0x%x\n", __func__, buffer[2]);
			if (buffer[2] & 0x4)
				pr_info("%s: Right SPK damaged event detected 0x%x\n", __func__, buffer[2]);
			break;
		}
	}
	...
	const struct i2c_device_id *id)
{
	...
	ret = device_create_file(&i2c->dev,&dev_attr_version);
	if (ret) {
